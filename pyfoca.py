...
import getopt, os, httplib, socket, urllib2, re, time, commands, sys, tqdm
...
banner = '\n ' + "-" * 79 + colors.white + '\n      . . . pyfoca v1.6.1 - Document Metadata Extractor, Kris Lloyd . . . \n ' + colors.normal + "-" * 79 + "\n"
class metaparser:
...
#                  print err
         ...
         for f in files:
            ...
            try:
               print " Attempting to download " + str(f)
               request = urllib2.Request(f, headers={'User-Agent' : "Magic Browser"})
               ...
               totalFiles -= 1
               errs.append(files[files.index(f)])
            except urllib2.URLError, e:
               print " " + str(e)
               print " Attempting to re-download " + str(f)
               try:
                       request = urllib2.Request(f, headers={'User-Agent' : "Magic Browser"})
                       response = urllib2.urlopen(request)
                       source = response.read()
                       ...
                       print " Tried 2 times, skipping..."
                       print colors.red + " [-] " + short_file, "-" * (spaces-len(short_file)), "fail", "[%s of %s]" % (str(files.index(f)+1),str(len(files))) + colors.normal
                       totalFiles -= 1
                       continue
         print
         for err in errs:
            del files[files.index(err)]
         for e in files:
